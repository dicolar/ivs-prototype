...
Utils.handleFormError = function(action) {
		...
		btns.push({
			...
			handler : function() {
				Ext.create('Ext.window.Window', {
					...
					html : '<pre style="font-weight:bold;font-size:16px;color:white;">' + action.result.errorDetail + '</pre>'
				}).show();
	...
	new top.Ext.window.Window({
		...
		items : {
			xtype : 'label',
			html : '<span style="color:white;">' + action.result.msg + '</span>'
		},
		buttons : btns
	}).show();
...
Utils.openLocator = function(locator, cfg) {
	...
	var cls = {
		ATTRIBUTELOCATOR : 'cn.incontent.locator.AttributeLocator',
		FOLDERLOCATOR : 'cn.incontent.locator.FolderLocator'
	}[locator];
